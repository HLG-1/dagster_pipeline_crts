...
class Trainer:
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                step_ok = self._run_step(
                    batch, phase, loss_mts, extra_loss_mts, raise_on_error=False
                )
                if not step_ok:
                    # Batch NaN/Inf : on saute l'optim step pour ne pas corrompre les poids
                    logging.warning(
                        "Skipping optimizer step after non-finite loss "
                        f"(epoch={self.epoch}, iter={data_iter})"
                    )
            ...
            # Catching NaN/Inf errors in the loss
                ...
                logging.error(f"FloatingPointError (continuing): {e}")
    ...
    def _run_step(
        ...
        Returns True if the step produced a finite loss and ran backward;
        False if the batch was skipped (NaN/Inf).
        """
        ...
        for i, chunked_batch in enumerate(batch):
            ...
            with ddp_context:
                ...
                if not math.isfinite(loss.item()):
                    error_msg = f"Loss is {loss.item()}, skipping batch (continue training)"
