...
def _do_matching(cost, repeats=1, return_tgt_indices=False, do_filtering=False):
    ...
    # NaN/Inf → crash scipy (« matrix contains invalid numeric entries »).
    # Remplace par un grand coût fini pour garder un matching dégradé plutôt
    # que d'interrompre tout le fine-tuning (vu en epoch 4 sur ce dataset).
    if not np.isfinite(cost).all():
        cost = np.nan_to_num(cost, copy=True, nan=1e9, posinf=1e9, neginf=1e9)
